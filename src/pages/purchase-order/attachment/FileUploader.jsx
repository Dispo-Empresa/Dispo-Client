...
const FileUploader = () => {
  ...
  const renderEmptyTemplate = () => (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <span style={{ fontSize: '1.2em', color: 'var(--text-color-secondary)', margin: '1rem 0' }}>
        Arraste e solte arquivos PDF aqui para fazer o upload.
      </span>
    </div>
  );

  const chooseButtonStyle = { background: '#029DBE', color: '#fff', borderRadius: '5px' };
  const uploadButtonStyle = { background: '#22C55E', color: '#fff', borderRadius: '5px' };
  const cancelButtonStyle = { background: '#F59E0B', color: '#fff', borderRadius: '5px' };
